...
## testando função 2 f(x) = 1/4000 * (SUM(xi**2))  - (PRODcos(xi/sqrt(i))) + 1
#def funcao_griewank(x):
#    i = np.array([i+1 for i in range(len(x))]).reshape(len(x),1)
#    return 1 + 1/4000 * (x**2).sum() - (np.cos(x/np.sqrt(i))).prod()

## testando na função f(x,y) = x**2 + y**2 + 1
#def funcao_teste(x):
#    return x[0]**2 + x[1]**2 + 1

# Criar a função do ABC
def ABC(fun, fitness, *args, tam_colonia = 20, atributos_dim, min, max, seed = 1, max_cycle):
    '''
    ...
    #    print("---------------------------")
        ...
        for j in np.arange(NP):
            ...
            v = fonte_alimentos[j, :] + r * (fonte_alimentos[j, :] - fonte_alimentos[k, :])
        ...
        for o in np.arange(NP):
            ...
            if (r1 < probs[o]):
                ...
                v = fonte_alimentos[o, :] + r * (fonte_alimentos[o, :] - fonte_alimentos[k, :])
...
for i in dims:
    ...
    for func in range(len(funcoes['funcoes'])):
        ...
        # Rodando o algoritmo
        fonte_alimentos, melhor_solucao, funcao_iteracao, media, desvio_pad = ABC(fun = funcoes['funcoes'][func], fitness = fitness, tam_colonia = 100, atributos_dim = i, 
        min = funcoes['dom_min'][func], max = funcoes['dom_max'][func], seed = 1, max_cycle = epocas)
        ...
        nome = "ABC_resultados/ABC_fig_"+str(str(funcoes['funcoes'][func])[10:16])+"_"+str(i)+'.png'
        plt.savefig(nome)
        plt.close()

        np.savetxt("ABC_resultados/ABC_"+str(str(funcoes['funcoes'][func])[10:16])+"_"+str(i)+".csv", fonte_alimentos[melhor_solucao, :], delimiter=",")
        resultados.to_csv("ABC_resultados/ABC_resultados_"+str(str(funcoes['funcoes'][func])[10:16])+"_"+str(i)+".csv")